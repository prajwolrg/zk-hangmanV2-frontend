...
function GamePage() {
  ...
  const connectContract = async () => {
    ...
    updateContractAddresses({
      ...contractAddreses,
      ZK_HANGMAN_GAME_ADDRESS: gameContract,
    });
    try {
      ...
      for (let i = 0; i < Math.floor(turn / 2); i++) {
        let guess = String.fromCharCode(
          parseInt(await zkHangmanContract.guesses(i)) + 97
        );
        console.log(guess);
        guesses.push(guess);
      }
      ...
      for (let i = 0; i < totalChars; i++) {
        let revealedChar =
          parseInt(await zkHangmanContract.revealedChars(i)) + 97;
        if (revealedChar < 123) {
