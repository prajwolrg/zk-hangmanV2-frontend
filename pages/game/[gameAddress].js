...
function GamePage() {
  ...
  const connectWallet = async () => {
    try {
      ...
      zkHangmanContract.on("NextTurn", async (nextTurn) => {
        ...
        let _correctGuesses = parseInt(
          await zkHangmanContract.correctGuesses(),
          16
        );
  ...
  const processGuess = async (e) => {
    ...
    let hexLatestGuess = await zkHangmanContract.guesses(
      toHex(Math.floor((turn - 1) / 2))
    );
    console.log("turn: ", turn);
  ...
  return (
    <>
      ...
      <VStack mt={10}>
        ...
        {correctGuesses == 5 && <h1> GAME OVER. PLAYER HAS WON! </h1>}
        {!contractConnected && <h1> Connecting to contract... </h1>}
        // // Characters revealed //
        {contractConnected &&
          (chainId == 1666900000 ||
            chainId == 1666600000 ||
            ...
            <>
              ...
              <VStack width={500}>
                {turn == 0 ? (
                  <Heading size="lg">
                    {" "}
                    Waiting for host to choose a word and initialize game{" "}
                  </Heading>
                ) : turn % 2 == 1 && account != playerAddress ? (
                  <Heading size="lg"> Player's turn! </Heading>
                ) : turn % 2 == 0 && account != hostAddress ? (
                  <Heading size="lg"> Host's turn! </Heading>
                ) : (
                  <> </>
                )}
              </VStack>
        ...
        {contractConnected &&
          ...
          turn % 2 == 0 && (
            <VStack width={500}>
              <Heading size="md">
                {" "}
                You are the host. Process the guess. If the value of the secret
                field below is empty, enter the secret you set in the init
                process!{" "}
              </Heading>
              <form onSubmit={processGuess}>
                <FormLabel>Secret number:</FormLabel>
                <Input
                  mb={3}
                  width={500}
                  type="number"
      ...
      <AlertDialog isOpen={isSelectOpen} onClose={onSelectClose}>
        <AlertDialogOverlay>
          <AlertDialogContent>
            <AlertDialogBody align="center" py={10}>
              <Button mb={7} width={250} onClick={() => switchNetwork(true)}>
                {" "}
                Connect to harmony mainnet{" "}
              </Button>
              <Button width={250} onClick={() => switchNetwork(false)}>
                {" "}
                Connect to harmony testnet{" "}
              </Button>
            </AlertDialogBody>
          </AlertDialogContent>
        </AlertDialogOverlay>
